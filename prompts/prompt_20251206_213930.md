...
- **Available Methods:**
  ...
  * `tao_hero` (constant: `TAO_HERO`)
  * `ren_kiem` (constant: `REN_KIEM`)
  * `trang_bi` (constant: `TRANG_BI`)
  * `thao_kiem` (constant: `THAO_KIEM`)

---

# **Move Contract: `contract/rpg_game/sources/hero.move`**
...
public entry fun tao_hero(ten_hero: vector<u8>, ctx: &mut TxContext) {
let hero = Hero {
id: object::new(ctx),
ten: ten_hero,
vu_khi: option::none()
...
public entry fun ren_kiem(suc_manh: u64, ctx: &mut TxContext) {
let kiem = Sword {
id: object::new(ctx),
suc_manh: suc_manh
};
transfer::public_transfer(kiem, tx_context::sender(ctx));
}
public entry fun trang_bi(hero: &mut Hero, kiem: Sword) {
assert!(option::is_none(&hero.vu_khi), 0);
option::fill(&mut hero.vu_khi, kiem);
}
public entry fun thao_kiem(hero: &mut Hero, ctx: &mut TxContext) {
assert!(option::is_some(&hero.vu_khi), 1);
let kiem = option::extract(&mut hero.vu_khi);
transfer::public_transfer(kiem, tx_context::sender(ctx));
}
}
```
...
- Use Package ID: `0xdf08de02aa3f7d30f115c88671a31c1cce2c9cd2a653b95aa7f5ee463da6274a` and Module: `pizza`
